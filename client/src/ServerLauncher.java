...
/**
 * Client side code that interacts with `Server.scala` in order to launch a
 * generic long-lived background server.
 *
 * The protocol is as follows:
 *
 * - Client: - Take clientLock - If processLock is not yet taken, it means
 * server is not running, so spawn a server - Wait for server socket to be
 * available for connection - Server: - Take processLock. - If already taken, it
 * means another server was running (e.g. spawned by a different client) so exit
 * immediately - Server: loop: - Listen for incoming client requests on
 * serverSocket - Execute client request - If clientLock is released during
 * execution, terminate server (otherwise we have no safe way of terminating the
 * in-process request, so the server may continue running for arbitrarily long
 * with no client attached) - Send `ProxyStream.END` packet and call
 * `clientSocket.close()` - Client: - Wait for `ProxyStream.END` packet or
 * `clientSocket.close()`, indicating server has finished execution and all data
 * has been received
 */
public abstract class ServerLauncher {
	...
	public ServerLauncher(InputStream stdin, PrintStream stdout, PrintStream stderr, Map<String, String> env,
			String[] args, Locks[] memoryLocks, int forceFailureForTestingMillisDelay) {
		this.stdin = stdin;
		this.stdout = stdout;
		this.stderr = stderr;
		this.env = env;
	...
	public Result acquireLocksAndRun(String outDir) throws Exception {
		...
		final String versionAndJvmHomeEncoding = Util.sha1Hash("0.12.3" + System.getProperty("java.home"));
		...
		while (serverIndex < serverProcessesLimit) { // Try each possible server process (-1 to -5)
			...
			try (Locks locks = memoryLocks != null ? memoryLocks[serverIndex - 1] : Locks.files(serverDir.toString());
