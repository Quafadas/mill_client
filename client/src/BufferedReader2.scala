...
class BufferedReader2(in: Reader, sz: Int) extends Reader {
  ...
  def lines(closeAtEnd: Boolean): Stream[String] = {
    val spliter =
  ...
  private def prepareRead(): Boolean = {
    println(s" ensureOpen : ${ensureOpen()}")
    println(s" pos : $pos")
    println(s" end : $end")
    println(s" buf : ${buf.mkString}")
    println(s" validMark : $validMark")
    println(pos < end)
    pos < end || fillBuffer()
  }
